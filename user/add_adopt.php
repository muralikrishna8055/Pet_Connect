...
if (isset($_POST['adopt_submit'])) {
    ...
    // Check if the uploads/adoption folder exists, and create it if not
    if (!file_exists('uploads/product/')) {
        mkdir('uploads/product/', 0777, true);
    }

    // Validate that all fields are filled
    if (!empty($pet_name) && !empty($breed) && !empty($age) && !empty($description) && !empty($contact_number) && !empty($pet_image)) {
        ...
        if ($_FILES['pet_image']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($image_tmp, $image_folder)) {
                ...
                $query = "INSERT INTO pets_for_adoption (pet_name, breed, age, description, contact_number, pet_image, stat,uid) 
                          VALUES ('$pet_name', '$breed', '$age', '$description', '$contact_number', '$pet_image', '$stat','$user_id')";
                $query_run = mysqli_query($connection, $query);

                if ($query_run) {
                    echo '<script>alert("Pet adoption details successfully added.");</script>';
                    echo '<script>location.replace("add_adopt.php");</script>';
                } else {
    ...
    <main id="main" class="main">
        ...
        <section class="section">
            <div class="row">
                ...
                <div class="card">
    <div class="card-body">
        <h5 class="card-title">Adoptable Pet Details</h5>
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <label class="col-sm-2 col-form-label">Breed<span class="text-danger">*</span></label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="breed" placeholder="Enter pet breed" required>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" name="description" rows="4" placeholder="Enter adoption description" required></textarea>
                </div>
            </div>
            ...
            <div class="text-center">
                <button type="submit" name="adopt_submit" class="btn btn-primary">List Pet for Adoption</button>
                <button type="reset" class="btn btn-secondary">Reset</button>
            </div>
        </form><!-- End Pet Adoption Form -->
